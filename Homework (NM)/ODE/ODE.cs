...
public static class ODE {  
    ...
    public static vector driver(
        ...
        // If lists for storing results are provided, initialize them
        if (xlist != null) { xlist.Clear(); xlist.Add(x); }
        if (ylist != null) { ylist.Clear(); ylist.Add(y); }

        // Adaptive step-size loop
        do {
            if (x >= b) return y; // Stop when reaching b
            ...
            if (err < tol) {
                ...
                if (xlist != null) xlist.Add(x); // Store step in list if provided
